...
### FIX Protocol — What It Is and Why It Exists

**FIX** (Financial Information eXchange) is the universal messaging standard for electronic trading. Created in 1992 by Fidelity and Salomon Brothers to replace phone-based order routing, it is now used by every major exchange, broker, and asset manager globally — NYSE, LSE, HKEX, CME, every investment bank, and every institutional fund manager speaks FIX.

#### The Problem FIX Solves

Before FIX, each broker had a proprietary API. A fund manager with 10 brokers needed 10 different integrations. FIX replaced all of them with a single standard.

Today, a hedge fund's OMS (Order Management System) connects to 50+ execution venues — all using FIX. A single standard cut integration cost from months to days.

#### How It Works

FIX messages are tag=value pairs delimited by `\x01` (SOH character):

```
8=FIX.4.2 | 9=176 | 35=D | 49=CLIENT1 | 56=EXCHANGE | 11=ORD001
54=1 | 55=0005.HK | 38=2000 | 40=2 | 44=68.20 | 10=042
```

Key fields (called "tags"):

| Tag | Name | Example | Meaning |
|---|---|---|---|
| 35 | MsgType | `D` | Message type (D=NewOrderSingle, 8=ExecutionReport, V=MarketDataRequest) |
| 49 | SenderCompID | `CLIENT1` | Who sent this message |
| 56 | TargetCompID | `EXCHANGE` | Who should receive it |
| 11 | ClOrdID | `ORD001` | Client's order reference — must be unique per day |
| 54 | Side | `1` | 1=Buy, 2=Sell, 5=SellShort |
| 55 | Symbol | `0005.HK` | Instrument identifier |
| 38 | OrderQty | `2000` | Quantity |
| 40 | OrdType | `2` | 1=Market, 2=Limit |
| 44 | Price | `68.20` | Limit price (only for limit orders) |

#### The FIX Session Layer

FIX manages its own TCP session, separate from the order messages:

```
Logon (35=A)          ← broker establishes session, sends sequence number
Heartbeat (35=0)      ← sent every N seconds to prove connection is alive
TestRequest (35=1)    ← one side asks "are you there?"
ResendRequest (35=2)  ← "I missed messages 15–20, please re-send"
Logout (35=5)         ← graceful disconnect
```

Every message has a sequence number (`MsgSeqNum`, tag 34). If a gap is detected, the receiver sends a `ResendRequest` to recover missed messages — this is how FIX guarantees delivery without relying on TCP retransmit alone.

#### The FIX Order Lifecycle

```
Client                              Exchange
──────                              ────────
NewOrderSingle (D)      ──────────► enterOrder()
                                       │
                        ◄──────────  ExecutionReport (8) OrdStatus=New
                                       │  (order accepted, sitting in book)
                                       │
                                    [match occurs]
                                       │
                        ◄──────────  ExecutionReport (8) OrdStatus=PartiallyFilled
                        ◄──────────  ExecutionReport (8) OrdStatus=Filled

OrderCancelRequest (F)  ──────────► cancelOrder()
                        ◄──────────  ExecutionReport (8) OrdStatus=Canceled
                           OR
                        ◄──────────  OrderCancelReject (9)  (e.g. order already filled)
```

The `ExecutionReport` (`35=8`) is FIX's workhorse — it communicates every state transition: acknowledged, partially filled, fully filled, cancelled, amended, rejected.

#### FIX Versions

| Version | Status | Key additions |
|---|---|---|
| FIX 4.0 | Legacy | First version, 1992 |
| FIX 4.2 | Widely deployed | Equity order types, market data messages |
| FIX 4.4 | Common | Cross-currency, derivatives improvements |
| FIX 5.0 | Modern | Transport-independent (FIXT 1.1 session) |
| FAST | Market data | Binary compression for high-volume MD feeds |
| ITCH | Proprietary | NASDAQ's ultra-low-latency market data protocol |

This simulator implements **FIX 4.2** — the version most commonly found at Asian exchanges (HKEX, SGX, ASX) for equity order routing.

#### QuickFIX/J — the FIX Engine

This project uses **QuickFIX/J** to handle the FIX session layer:
- TCP connection management
- Message parsing and validation against the FIX 4.2 data dictionary
- Sequence number tracking and gap recovery
- Heartbeat / TestRequest handling

`ExchangeFixGateway` creates the `SocketAcceptor` (the FIX server). `ExchangeFixManager` is the `Application` callback — it receives decoded, validated FIX messages and translates them into `Exchange` API calls. All the session plumbing (sequencing, resend, heartbeat) is handled by QuickFIX/J automatically.
