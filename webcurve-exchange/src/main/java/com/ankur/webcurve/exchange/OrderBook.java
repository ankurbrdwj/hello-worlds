...
/**
 * Industry term: Limit Order Book (LOB)
 *
 * The Limit Order Book is the heart of any exchange. It maintains two sorted lists:
 * - Bid orders: descending by price (best bid at index 0)
 * - Ask orders: ascending by price (best ask at index 0)
 *
 * When a new order arrives, the matching engine walks the opposite side looking for
 * price overlap — this is "price-time priority" (FIFO within the same price level).
 * Insertion uses binary search for O(log n) placement.
 *
 * In production, venues like CME and LSE implement LOBs as red-black trees or
 * skip lists to achieve O(1) best-price access and O(log n) inserts. The Level 2
 * market data that professional traders subscribe to is a snapshot of the LOB.
 *
 * VWAP (Volume Weighted Average Price) tracked here is the standard institutional
 * benchmark — traders use it to assess whether their execution got a fair average
