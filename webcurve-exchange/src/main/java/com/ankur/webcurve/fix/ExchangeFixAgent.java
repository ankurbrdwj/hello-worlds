...
import com.ankur.webcurve.client.Execution;
import com.ankur.webcurve.client.ExecutionListener;
...
import com.ankur.webcurve.common.MultiSubscriptionManager;
...
/**
 * Industry term: Buy-Side FIX Client / Broker Order Router
 *
 * Where ExchangeFixGateway is the exchange (sell-side acceptor), ExchangeFixAgent
 * is the client (buy-side initiator) — a broker or algorithmic trading system
 * that connects TO the exchange and submits orders over FIX 4.2.
 *
 * In the industry, this role is played by:
 * - OMS (Order Management System): Bloomberg AIM, Charles River, Flextrade —
 *   manages the full order lifecycle from portfolio manager through to execution
 * - EMS (Execution Management System): Fidessa, TradingScreen, FlexTrade —
 *   focused on real-time execution across multiple venues
 * - Algo engines: proprietary systems at hedge funds and prop trading firms
 *   that connect directly to exchange gateways for DMA (Direct Market Access)
 *
 * This agent also subscribes to MarketDataRequest (FIX MsgType=V) to receive
 * live order book snapshots (MarketDataSnapshotFullRefresh, W) from the exchange —
 * standard practice for algo strategies that need real-time depth-of-book data.
 * It uses MultiSubscriptionManager to fan out market data to multiple consumers.
 */
@Slf4j
