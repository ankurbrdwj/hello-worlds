...
-- ============================================================
SELECT p.clientid, p.model AS active_model, p.date AS since
FROM phone_log p
         INNER JOIN (
    SELECT clientid, MAX(date) AS latest_date
    FROM phone_log
    GROUP BY clientid
) latest ON p.clientid = latest.clientid AND p.date = latest.latest_date
